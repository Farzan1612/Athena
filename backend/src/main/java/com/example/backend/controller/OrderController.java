...
@CrossOrigin(origins = {"http://localhost:5173", "http://127.0.0.1:5173"}, allowCredentials = "true", methods = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE, RequestMethod.OPTIONS}, allowedHeaders = "*")
public class OrderController {
    ...
    public ResponseEntity<List<Map<String, Object>>> getAllOrders(
        ...
        try {
            System.out.println("Fetching all orders - page: " + page + ", size: " + size + ", sort: " + sort);
            ...
            for (Order order : orders) {
                ...
                simplifiedOrder.put("userId", order.getUser().getUserId());
                ...
                simplifiedOrder.put("customerName", order.getCustomerName());
                simplifiedOrder.put("email", order.getEmail());
                
                List<Map<String, Object>> simplifiedDetails = new ArrayList<>();
                for (OrderDetail detail : order.getOrderDetails()) {
                    Map<String, Object> simplifiedDetail = new HashMap<>();
                    simplifiedDetail.put("orderDetailId", detail.getOrderDetailId());
                    simplifiedDetail.put("productId", detail.getProduct().getProductId());
                    simplifiedDetail.put("productName", detail.getProduct().getName());
                    simplifiedDetail.put("quantity", detail.getQuantity());
                    simplifiedDetail.put("price", detail.getPrice());
                    simplifiedDetails.add(simplifiedDetail);
                }
                
                simplifiedOrder.put("orderDetails", simplifiedDetails);
                simplifiedOrders.add(simplifiedOrder);
            }
            ...
            e.printStackTrace();
            return ResponseEntity.status(500).body(new ArrayList<>()); // Return empty array instead of null
        }
    }
