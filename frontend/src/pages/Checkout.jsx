  ...
  const handlePayment = async (total, orderId) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/payments/razorpay/orders/${total.toFixed(2)}`,
        {
          headers: {
      ...
      const options = {
        key: "rzp_test_MMii1ZKBMNbIk6", // Updated to match backend configuration
      ...
      console.error("Payment Error Details:", {
        ...
        url: `http://localhost:8080/api/payments/razorpay/orders/${total.toFixed(2)}`,
        fullError: error
      });
      
      // First, test if the basic API endpoint is accessible
      try {
        const testResponse = await axios.get('http://localhost:8080/api/payments/test');
