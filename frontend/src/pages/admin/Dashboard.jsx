...
const Dashboard = () => {
  ...
  const fetchDashboardData = async (showLoading = true) => {
    try {
      ...
      // Updated to handle the simplified order data returned by the backend
      ...
      // Ensure all orders have the expected fields
      const normalizedOrders = ordersData.map(order => ({
        ...
        customerName: order.customerName || (order.user && order.user.username) || "Guest",
              ...
              {recentOrders.map(order => {
                ...
                return (
                  <tr key={order.orderId || `order-${Math.random()}`}>
                    <td>#{order.orderId}</td>
                    <td>{order.customerName || (order.user && order.user.username) || "Guest"}</td>
